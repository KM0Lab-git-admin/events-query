...
-- ============================================================================
-- Versión: 4.0 UNIFICADA (API + ingesta + RECINTOS; alinear con SQL/SCHEMA_SQL_FINAL.sql)
-- Fecha: Mayo 2026
--
-- Incluye:
...
--
-- Este fichero es el que monta docker-compose como docker-entrypoint-initdb.d (BD vacía).
-- ============================================================================
...
-- Sesión UTF-8 para literales del script (evita mojibake al cargar desde docker-entrypoint-initdb.d).
...
-- Ciudades y CP mínimos para ingesta KM0 (Malgrat id=1, Blanes id=2; alineado con CSVs típicos)
INSERT INTO `CIUDADES` (`ID_Ciudad`, `Nombre`, `Provincia`, `Latitud`, `Longitud`) VALUES
  (1, 'Malgrat de Mar', 'Girona', 41.64670000, 2.74140000),
  (2, 'Blanes', 'Girona', 41.67530000, 2.79250000)
...
INSERT INTO `CODIGOS_POSTALES` (`CP`, `ID_Ciudad`, `Latitud`, `Longitud`) VALUES
  ('08380', 1, 41.64670000, 2.74140000),
  ('17300', 2, 41.67530000, 2.79250000)
ON DUPLICATE KEY UPDATE
...
CREATE TABLE IF NOT EXISTS `EVENTOS_MASTER` (
  ...
  `Desc_Corta_CAT` VARCHAR(500) NULL COMMENT 'Resumen 2-3 frases para tarjetas (generado por LLM)',
  `Desc_Corta_ES` VARCHAR(500) NULL COMMENT 'Resumen 2-3 frases para tarjetas (generado por LLM)',
  `Tags_ES` JSON NULL,
  `Tags_CAT` JSON NULL,
...
CREATE TABLE IF NOT EXISTS `NOTICIAS_MASTER` (
  ...
  `Cuerpo_ES` TEXT NULL,
  `Resumen_CAT` VARCHAR(1200) NULL COMMENT 'Resumen max 3 frases/parrafos cortos (LLM); es lo persistido',
  `Resumen_ES` VARCHAR(1200) NULL COMMENT 'Resumen max 3 frases/parrafos cortos (LLM); es lo persistido',
  `Tags_CAT` JSON NULL,
  `Tags_ES` JSON NULL,
...
-- ============================================================================
-- INGESTA_RUNS + INGESTA_COSTES: gasto de OpenAI por ejecución (API/front)
-- ============================================================================
